...
// Puppeteer with Sock5 Different ports ( for proxy ) -- app used 
// tryCount multiple tabs --- with tryCoung multiple browsers --- no limit of failures
        ...
        if(isGoogleCaptcha)
        {
            ...
            try
            {
                ...
                if(tryCount===3)  //at 4th iteration
                {
                    ...
                    const args = [ "--no-sandbox",'--disable-setuid-sandbox',`--proxy-server=socks5://127.0.0.1:${9050 + currPort+1}` ]
                    
                    const newBrowser = await puppeteer.launch({ headless: true,args,slowMo:10,defaultViewport:null })
...
async function scrape(website,location,industry,domain,extra,extraLoc,pageNumber,app)
{
    // console.log(app);
    ...
    let currPort = parseInt(app.get('currPort'));
    console.log(`-------- Active Port: ${currPort} -------------`)
    const args = [ "--no-sandbox",'--disable-setuid-sandbox',`--proxy-server=socks5://127.0.0.1:${9050 + currPort}` ]
    ...
    await navigation(browser,page,url,true,0,app)
